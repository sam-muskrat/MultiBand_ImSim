...
def run_task_1_simulate(configs_dict, Nmax_proc, rng_seed, g_cosmic, g_columns, run_tag, needed_tile, stars_only):
    ...
    input_folder = configs_dict['work_dirs']['cata_folder_names'][0]
...
def run_task_2_swarp(configs_dict, tile_labels, Nmax_proc, running_log, needed_tile):
    """Task 2: swarp images."""
    import Astromatic
...
def run_task_3_detect(configs_dict, tile_labels, N_tiles, Nmax_proc, running_log, needed_tile):
    ...
    input_folder, detect_foler, CrossMatch_folder = configs_dict['work_dirs']['cata_folder_names'][:3]
...
def run_task_4_photometry(configs_dict, tile_labels, Nmax_proc, running_log, needed_tile):
    """Task 4: measure photometry."""
    logger.info('====== Task 4: measure photometry === started ======')
    start_time = time.time()

    input_folder, detect_foler = configs_dict['work_dirs']['cata_folder_names'][:2]
    photometry_folder = configs_dict['work_dirs']['cata_folder_names'][3]
...
def run_task_5_photoz(configs_dict, tile_labels, Nmax_proc, running_log, needed_tile):
    """Task 5: measure photo-z."""
    logger.info('====== Task 5: measure photo-z === started ======')
    start_time = time.time()

    detect_foler = configs_dict['work_dirs']['cata_folder_names'][1]
    photometry_folder = configs_dict['work_dirs']['cata_folder_names'][3]
    photo_z_folder = configs_dict['work_dirs']['cata_folder_names'][4]
...
def run_task_6_1_psfmodel(configs_dict, tile_labels, Nmax_proc, running_log, needed_tile, varChips_dic):
    """Task 6_1: PSF modelling."""
    import PSFmodelling
...
def run_task_6_2_shapes(configs_dict, tile_labels, Nmax_proc, rng_seed, running_log, needed_tile, PSF_size_error=0.0, PSF_beta_error=0.0):
    ...
    detect_foler = configs_dict['work_dirs']['cata_folder_names'][1]
    shapes_folder = configs_dict['work_dirs']['cata_folder_names'][5]
    ...
    if configs_dict['MS']['method'].lower() == 'lensfit':
        ...
        psf_error_tag = ''
        if PSF_size_error != 0.:
            psf_error_tag += f'_PSF_size_error_{PSF_size_error:+.4f}'
        if PSF_beta_error != 0.:
            psf_error_tag += f'_PSF_beta_error_{PSF_beta_error:+.4f}'
        if psf_error_tag:
            out_dir_tmp = os.path.join(out_dir_tmp, psf_error_tag.lstrip('_'))
        pathlib.Path(out_dir_tmp).mkdir(parents=True, exist_ok=True)
        ...
        ## PSF size/beta error: regenerate a biased Moffat PSF for MetaDetect only
        ##  (the images/PSF used for simulation are untouched)
        if (PSF_size_error != 0.) or (PSF_beta_error != 0.):
            logger.info(f'PSF_size_error={PSF_size_error}, PSF_beta_error={PSF_beta_error} requested: '
                        f'MetaDetect will use a PSF with {1.+PSF_size_error:.4f}x the true seeing '
                        f'and {1.+PSF_beta_error:.4f}x the true beta.')
        ...
        for i_band, band in enumerate(configs_dict['MS']['bands']):
            ...
            if (PSF_size_error != 0.) or (PSF_beta_error != 0.):
                ...
                if psf_type_band.lower() != 'moffat':
                    raise Exception(f'PSF_size_error/PSF_beta_error is only supported for Moffat PSFs (band {band} uses {psf_type_band})!')
                image_type_band = configs_dict['imsim']['image_type_list'][i_band_imsim].lower()
                if image_type_band in ('diffexpo', 'varchips'):
                    raise Exception(f'PSF_size_error/PSF_beta_error is not supported for diffExpo/varChips bands (band {band})!')
                ...
                for gal_rotation_angle in configs_dict['imsim']['gal_rotation_angles']:
                    ...
                    if configs_dict['MS']['metadetect_same_PSF']:
                        ...
                        if (PSF_size_error != 0.) or (PSF_beta_error != 0.):
                            ...
                            beta_detect = beta_true * (1. + PSF_beta_error)

                            inpath_psf_image = os.path.join(in_ima_dir_tmp,
                                                        f'psf_tile{tile_label}_band{band}',
                                                        f'psf_ima_detect{psf_error_tag}.fits')
                            if not os.path.isfile(inpath_psf_image):
                                PSF_detect = ImSimPSF.MoffatPSF(seeing_detect, beta_detect, psf_e)
                                ## match whichever flavour metadetect_psf_image selects, so the only
                                ##  thing perturbed is the seeing/beta, not also the PSF centering
                                psf_ima_detect = ImSimPSF.PSFima(PSF_detect, pixel_scale_sim,
                                                    size=configs_dict['imsim']['image_PSF_size'], pixelPSF=False,
                                                    half_pixel_shift=(configs_dict['MS']['metadetect_psf_image'] != 'centred'))
                                psf_ima_detect.write(inpath_psf_image)
                                logger.info(f'Biased PSF image saved as {inpath_psf_image}')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--PSF_beta_error', type=float, default=0.0, metavar='PSF_beta_error',
        help="Percent error (as a fraction) applied to the Moffat PSF 'beta' used\n"
             "   by MetaDetect during shape measurement only; image simulation is\n"
             "   unaffected. E.g. 0.01 -> MetaDetect sees a PSF with 1.01x the true beta.\n"
             "   (default: %(default)s, i.e. no PSF beta error)")
    ...
    PSF_beta_error = args.PSF_beta_error
    ...
    if ('2' in taskIDs) or ('all' in taskIDs):
        run_task_2_swarp(configs_dict, tile_labels, Nmax_proc, running_log, needed_tile)

    # 3: detect objects
    if ('3' in taskIDs) or ('all' in taskIDs):
        run_task_3_detect(configs_dict, tile_labels, N_tiles, Nmax_proc, running_log, needed_tile)

    # 4: measure photometry
    if ('4' in taskIDs) or ('all' in taskIDs):
        run_task_4_photometry(configs_dict, tile_labels, Nmax_proc, running_log, needed_tile)

    # 5: measure photo-z
    if ('5' in taskIDs) or ('all' in taskIDs):
        run_task_5_photoz(configs_dict, tile_labels, Nmax_proc, running_log, needed_tile)

    # 6_1: PSF modelling
    if ('6_1' in taskIDs) or ('all' in taskIDs):
        run_task_6_1_psfmodel(configs_dict, tile_labels, Nmax_proc, running_log, needed_tile, varChips_dic)

    # 6_2: measure galaxy shapes
    if ('6_2' in taskIDs) or ('all' in taskIDs):
        run_task_6_2_shapes(configs_dict, tile_labels, Nmax_proc, rng_seed, running_log, needed_tile, PSF_size_error, PSF_beta_error)


    # 7: create a combined catalogue
    if ('7' in taskIDs) or ('all' in taskIDs):
        run_task_7_combine(configs_dict, tile_labels, needed_tile)
